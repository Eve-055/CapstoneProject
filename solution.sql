  ...
  -- Task 2: Find all orders placed in the last 30 days with a quantity greater than 1, displaying order_id, order_date, and total cost (cost * quantity).
   ...
   -- Task 4: Delete all bookings that are more than 1 year old and were made by customers who haven't placed any orders
              SELECT* FROM bookings;        SELECT* FROM customers;       SELECT* FROM orders;
 ...
 -- Task 5: Calculate the average preparation time for each course type (starter, main course, dessert) across all menus.
               SELECT*FROM menus;    SELECT*FROM courses;   SELECT*FROM starters;
 ...
 JOIN courses c ON m.course = c.name
  ...
  -- Task 7: Find the most popular menu item (based on order count) for each cuisine type.
              -- SELECT* FROM orders;    SELECT* FROM menus;
...
WHERE rank_within_cuisine = 1;
  ...
  ORDER BY 2 DESC;
   ...
   LEFT JOIN 
    LittleLemonDB.menus m ON o.menu_name = m.menu_name
   LEFT JOIN 
    LittleLemonDB.starters s ON m.starter = s.name
   LEFT JOIN 
    LittleLemonDB.courses c ON m.course = c.name
   LEFT JOIN 
    LittleLemonDB.desserts d ON m.dessert = d.name
   LEFT JOIN 
    LittleLemonDB.drinks dr ON m.drink = dr.name
   LEFT JOIN 
    LittleLemonDB.sides si ON m.side = si.name
  ORDER BY 
  ...
  -- Task 11: Create a view that shows staff performance metrics including number of orders taken,total sales generated, and average order value.
                -- SELECT * FROM orders;   SELECT * FROM staff;
   ...
   DROP PROCEDURE IF EXISTS BusiestBookingTimes;

  DELIMITER //

  CREATE PROCEDURE BusiestBookingTimes()
  BEGIN
    DECLARE min_date DATE;
    DECLARE max_date DATE;

    -- Get the min and max booking dates
    SELECT MIN(booking_date), MAX(booking_date)
    INTO min_date, max_date
    FROM bookings;

    -- Return booking counts grouped by hour
    SELECT 
 ...
 -- Task 21: Rewrite one of your complex queries using a CTE instead of subqueries and explain the benefits.
  ...
  rank_within_cuisine = 1;
   ...
   /* 
    1. Advantages of CTE over subquery:
    2. Cleaner and modular	
    3. Can be used in the same query
    4. Supports recursive query
    5. Easier to manage step by step 
  */
  ...
  -- Task 24: Create a comprehensive report that combines data from all tables to show restaurant performance metrics, including customer acquisition and retention rates; Table turnover rates; Menu item profitability; Staff productivity */
  ...
  FROM acquisition a
  LEFT JOIN retention r ON r.months_active > 1
  ...
  JOIN LittleLemonDB.menus m ON o.menu_name = m.menu_name
  GROUP BY m.menu_name
  ORDER BY total_profit DESC;

     -- staff productivity
              -- SELECT * FROM staff;    SELECT * FROM orders;
...
from a variable. The statement should return the order id, the quantity and the order cost from the Orders table.*/
  ...
  -- Task 27: Create a stored procedure called CancelOrder. Little Lemon want to use this stored procedure to delete an order record based on the user input of the order id.
...
The procedure should include four input parameters in the form of the following bookings parameters:booking id,customer id,booking date and table numberAddBooking. 
 ...
 END //
  ...
  DELIMITER ;
               To confirm if the above query has executed and to get the output, use:
...
an UPDATE statement inside the procedure. */
 ...
 END //
  ...
  -- Task 32: Create a new procedure called CancelBooking that they can use to cancel or remove a booking. The procedure should have one input parameter in the form of booking id. You must also write a DELETE statement inside the procedure.*/
